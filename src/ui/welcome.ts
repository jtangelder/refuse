...
@Component({
  ...
  template: `
    ...
    <div class="message">
      ...
      <p>
        It provides a simple interface to manage your presets, configure signal
        chains, and access the "hidden" amp models that require software
        control. No installation is needed; it runs entirely via your browser's
        USB connection.
      </p>
      ...
      <div class="connect">
        <button (click)="api.connect()" class="success">
          <span class="icon">usb</span>
          Connect your amp
        </button>
      </div>

      <p *ngIf="!api.isSupported" class="unsupported">
        ⚠️ WebHID not detected. This application requires a browser with WebHID
        support (<a href="https://caniuse.com/webhid" target="_blank"
