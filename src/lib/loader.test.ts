...
describe('FuseLoader', () => {
  ...
  it('should parse preset name and amplifier settings', async () => {
    ...
    expect(api.sendFullBuffer).toHaveBeenCalledWith(DspType.AMP, 0, expect.any(Uint8Array));
    expect(api.sendFullBuffer).toHaveBeenCalledWith(DspType.STOMP, 0, expect.any(Uint8Array));
  });

  it('should ignore unknown tags', async () => {
    const xmlWithExtra = mockPresetXml.replace('</FenderPreset>', '<UnknownTag><Module ID="99" POS="0"/></UnknownTag></FenderPreset>');
